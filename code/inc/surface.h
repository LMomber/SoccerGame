...
namespace Tmpl8 {
	...
	inline Pixel AddBlend(Pixel a_Color1, Pixel a_Color2)
	{
		...
		// If color << 8 has a value > 1, color1 = 255, else color1 = color & mask.
	...
	inline Pixel SubBlend(Pixel a_Color1, Pixel a_Color2)
	{
		...
		// If the color value is < 0, make 0.
		if (red < 0) red = 0;
		if (green < 0) green = 0;
		if (blue < 0) blue = 0;
	...
	public:
		...
		void LoadImages(const char* a_File);
	...
	public:
		Font();
		Font(const char* a_File, char* a_Chars);
		~Font();
